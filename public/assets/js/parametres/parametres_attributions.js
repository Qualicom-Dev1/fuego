$(document).ready(() => {
    ...
    $('.role_item').click( event => {
        ...
        $.ajax({
            url: '/parametres/attributions/get-dependence',
            method: 'POST',
            data: {
            ...
            data.findedDependences.Structuresdependences.forEach(element => {
                $('#idprivilege_'+element.idUser).addClass('active')
            });
        });
    ...
    $('.fa-check').click( () => {
        ...
        $.ajax({
            url: '/parametres/attributions/set-dependence',
            method: 'POST',
            data: data
        }).then(data => {
            $('.privilege_item').removeClass('active')
