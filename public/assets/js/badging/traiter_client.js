...
$(".telec_boutons button").click((event) => {
    ...
    if(!$(event.currentTarget).hasClass('hangup') && !$(event.currentTarget).hasClass('appel')){
        ...
        if($(event.currentTarget).attr('id').split('_')[0] != 'action'){
            ...
            let phase2_extend = ''
            if(urlTraitement === 'rdv.ejs') {
                phase2_extend = new EJS({ url: '/public/views/partials/badging/'+urlTraitement}).render();
            }
            else {
                phase2_extend = new EJS({ url: '/public/views/partials/traitementclient/'+urlTraitement}).render();
            }
